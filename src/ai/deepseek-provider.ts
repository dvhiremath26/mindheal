  ...
  async suggestLocator(request: AIHealingRequest): Promise<AIHealingResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `DeepSeek API error (${response.status}): ${errorResp.error?.message ?? response.statusText}`
        );
      }
    } catch (error: unknown) {
      if (error instanceof DOMException && error.name === 'AbortError') {
        throw new Error(
          `[MindHeal] DeepSeek API request timed out after ${REQUEST_TIMEOUT_MS}ms`
        );
      }
      ...
      throw new Error(
        `[MindHeal] DeepSeek API request failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
