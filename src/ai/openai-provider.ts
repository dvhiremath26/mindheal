...
export class OpenAIProvider implements AIProvider {
  ...
  async suggestLocator(request: AIHealingRequest): Promise<AIHealingResponse> {
    ...
    let responseData: OpenAISuccessResponse | OpenAIErrorResponse;
    ...
    try {
      ...
      responseData = (await response.json()) as OpenAISuccessResponse | OpenAIErrorResponse;

      if (!response.ok) {
        const errorResp = responseData as OpenAIErrorResponse;
        throw new Error(
          `OpenAI API error (${response.status}): ${errorResp.error?.message ?? response.statusText}`
        );
      }
    } catch (error: unknown) {
      if (error instanceof DOMException && error.name === 'AbortError') {
        throw new Error(
          `[MindHeal] OpenAI API request timed out after ${REQUEST_TIMEOUT_MS}ms`
        );
      }
      ...
      throw new Error(
        `[MindHeal] OpenAI API request failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }

    const successResp = responseData as OpenAISuccessResponse;
